...
public class servletMasterData extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public servletMasterData() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			sbSql.append(" SELECT * FROM xpush_grid1 ORDER BY \"COL1\"");
			pstmt = conn.prepareStatement(sbSql.toString());
			pstmt.setFetchSize(1000);
			rs = pstmt.executeQuery();

			DataSet ds = util.getDataset(rs, "output1");

			outpd.addDataSet(ds);
			
			sbSql = new StringBuffer();
			sbSql.append(" SELECT * FROM xpush_grid2 ORDER BY \"COL1\"");
			pstmt = conn.prepareStatement(sbSql.toString());
			pstmt.setFetchSize(1000);
			rs = pstmt.executeQuery();

			ds = null;
			ds = util.getDataset(rs, "output2");
