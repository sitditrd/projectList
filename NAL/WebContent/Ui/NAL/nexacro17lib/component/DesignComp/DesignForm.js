		...
		if (rootobj) {
			...
			for (var i = comp_len - 1; i >= 0; i--) {
				...
				if (hit) {
					...
					if (this.hitTestException(comp) || !recursive) {
						return this._getScopeName(comp);
					}
	...
	_pDesignForm.setInnerDataset = function (compid, value, extern) {
		try {
			...
			if (obj.innerdataset == "innerdataset") {
				innerdataset = obj._innerdataset;
				delete innerdataset;
			...
			if (extern) {
				...
				innerdataset = new nexacro.NormalDataset(innerdatasetid, obj);
				...
				if (obj.set_innerdataset) {
					...
					traceD("obj(" + obj + ":" + obj.name + ") have no method set_innerdataset!!!");
				}
			}
		}
		catch (e) {
