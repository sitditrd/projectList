...
if (!JsNamespace.exist("nxlib"))
{
	...
	JsNamespace.declare("nxlib",
	{
		...
		getDsParam: function(form, sKey)
		{
			var ds = nxlib.lookup(form, "_ds_param");
			return ds.getColumn(0, sKey);
		},

		setDsParam: function(form, sKey, sValue, sColType, iColSize)
		{
			var ds = nxlib.lookup(form, "_ds_param");
			var colType = "string", colSize = 256;
			...
			if (ds.getColumnInfo(sKey))
			{
				ds.addColumn(sKey, colType, colSize);
			}

			ds.setColumn(0, sKey, sValue);
		},

		getFunctionID: function(form)
		{
			var ds = nxlib.lookup(form, "_ds_function");
			return ds && ds.getColumn(ds.rowposition, "functionid");
		},

		getMeasureList: function(form, objDS, sFunctionID, callback)
		{
			var oDatas = {
				svcid: "searchMeasureList",
				url: "common/measure/getMeasure.do",
				outds: [
					objDS.name+"=outds"
				]
			};

			nxlib.setDsParam(form, "functionid", sFunctionID);
			nxlib.setDsParam(form, "measuregbn", "MeasureList");
			nxlib.setDsParam(form, "isdefault", "Y");

			nxlib.transaction(form, oDatas, callback);
		}
	});
}