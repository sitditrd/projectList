...
try {
	...
	try {	
		...
		/** 쿼리 실행  **/
	  	...
	  	sql.append("  SELECT * FROM DIMENSIONINFO ");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"ds_DIMENSIONINFO"));	
		 
		sql = new StringBuffer();
	 	sql.append("  SELECT * FROM DIMENSIONBODYINFO ");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"ds_DIMENSIONBODYINFO"));	
		
		sql = new StringBuffer();
	 	sql.append("  SELECT * FROM POC_PGITABLE ");
	 	sql.append("  WHERE ROWNUM <= 1000");
	 	sql.append("  ORDER BY 1,2,3,4");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"ds_POC_PGITABLE"));
