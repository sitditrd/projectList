...
try {
	...
	try {	
		...
		/** 쿼리 실행  **/
	  	...
	  	sql.append("  SELECT ROWNUM AS CODE, A.MANAGER_ID AS CAPTION FROM ( SELECT DISTINCT(NVL(MANAGER_ID, 0)) MANAGER_ID FROM POC_TACTTIME ORDER BY 1)A ");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"dsOutput"));	
		
		sql = new StringBuffer();
		sql.append("  SELECT ROWNUM AS CODE, A.DEPARTMENT_ID AS CAPTION FROM ( SELECT DISTINCT(NVL(DEPARTMENT_ID, 0)) DEPARTMENT_ID FROM POC_TACTTIME ORDER BY 1)A ");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"dsOutput2"));	
		
		sql = new StringBuffer();
		sql.append("  SELECT FIRST_NAME, MANAGER_ID, 0 AS CHK FROM POC_TACTTIME ");
	  	rs = stmt.executeQuery(sql.toString());	
		pdata.addDataSet(makeDataSet(rs,"dsOutput3"));	
		
		sql = new StringBuffer();
		sql.append("  SELECT LAST_NAME, DEPARTMENT_ID, 0 AS CHK FROM POC_TACTTIME ");
	  	rs = stmt.executeQuery(sql.toString());	
