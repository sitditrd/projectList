...
if (!nexacro.JSONAdp) {
    ...
    var _pJSONAdp = nexacro._createPrototype(nexacro.Object, nexacro.ProtocolAdp);
    ...
    _pJSONAdp._rs_ = String.fromCharCode(30); //Record Separator 변수 생성
    _pJSONAdp._cs_ = String.fromCharCode(31); //Unit Separator 변수 생성
...
//         };
		...
		for(var i=0; i<ssvLines.length; i++)
		{
			trace("ssvLines[" + i + "] : " + ssvLines[i] + "\n");
		}
		
        for (var curIdx = 1; curIdx < lineCnt; curIdx++) { // SSV:utf-8은 fix
            ...
            if (curStr.substring(0, 7) != "Dataset") {
                ...
                if (strParam.length > 0) {
                    ...
                    oJsonData.data.parameters = JSON.parse(strParam);
                    strParam = "";
                }
                // parameters end
                ...
                if (nDsCnt > 0) { // dataset을 한 번이라도 처리한 경우 추가
                    ...
                    oJsonData.data.datasets.push(oJsonDataset);
