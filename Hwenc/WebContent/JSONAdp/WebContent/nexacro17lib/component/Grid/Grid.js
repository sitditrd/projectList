...
if (!nexacro.Grid) {
	...
	_pGrid._getCellStyleInfo = function (cellidx, prop, datarow, selected, parent_cellinfo) {
		...
		while (true) {
			...
			if (!obj) {
				if (!flush) {
					if (prop == "background" || prop == "cursor" || prop == "align" || prop == "textDecoration" || prop == "exportalign") {
						if (this._is_created) {
							if (!cell._is_created) {
								cell.on_created();
							}
				...
				else if (prop == "exportalign") {
					var tree = target_cell._subComp;

					if (tree && tree instanceof nexacro._CellTreeItemControl) {
						var exportalign = tree._getCurrentStyleAlign(true);
						obj = exportalign.textAlign + "," + exportalign.verticalAlign;
					}
					else {
						var align = target_cell._getCurrentStyleAlign(true);
						obj = align.textAlign + "," + align.verticalAlign;
