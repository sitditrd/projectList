...
class HealthRate extends Health {
	...
	private double standardHealth(){
		...
		SW = (this.height-100)*0.9;
		
		resultBI = (this.weight-SW)/SW*100;
	...
	private String rateCheck(){
		...
		if(this.standardHealth() < 10)
			chk = "정상";
		else if(this.standardHealth() >= 10 && this.standardHealth() <= 20)
			chk = "과체중";
		else if(this.standardHealth() >= 20)
			chk = "비만";
