...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#seachList').change(function() {
			$(".searchOptionIdx").attr('value', $("#seachList option").index($("#seachList option:selected")));
	...
	<div class="container">
		<div class="form-group col-md-4 div-align-middle">
			<form id="boardSearch" name="boardSearch">
				<select class="form-control" id="seachList" name="seachList">
				...
				<c:forEach items="${requestScope.aList}" var="dto">
					<tr>
						...
						<td class="boardSubject"><nobr>
								...
								<c:if
									test="${dto.member_code ne sessionScope.member_code && empty sessionScope.adminOk}">
									<a onclick="warning()">${dto.class_name}</a>
								</c:if>
								<c:if
									test="${dto.member_code eq sessionScope.member_code || not empty sessionScope.adminOk}">
				...
				<ul class="pagination justify-content-center">
					...
					<c:if test="${pdto.startPage > pdto.blockPage}">
						...
						<li class="page-item"><a class="page-link"
							aria-label="Previous"
							href="board.do?pageNum=${pdto.startPage - pdto.blockPage}"> <span
