		...
		var _bind_container_scroll_handler = function (elem) {
			return function (evt) {
				...
				if (elem._use_translate_scroll) {
					...
					var hpos = target.scrollLeft;
					var vpos = target.scrollTop;
					if (elem._scroll_top != target.scrollTop || elem._scroll_left != target.scrollLeft) {
						elem._callback_func_onscroll(evt);
					}
					else {
						elem._scroll_left = hpos;
						elem._scroll_top = vpos;
