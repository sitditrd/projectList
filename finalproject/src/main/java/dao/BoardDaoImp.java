package dao;

import java.util.List;

import org.mybatis.spring.SqlSessionTemplate;

import dto.BoardDTO;
import dto.BoardPageDTO;

public class BoardDaoImp implements BoardDAO {

	private SqlSessionTemplate sqlSession;

	public BoardDaoImp() {

	}

	public void setSqlSession(SqlSessionTemplate sqlSession) {
		this.sqlSession = sqlSession;
	}

	@Override
	public int count(BoardPageDTO pv) {
		return sqlSession.selectOne("board.count",pv);
	}

	@Override
	public List<BoardDTO> list(BoardPageDTO pv) {
		return sqlSession.selectList("board.list", pv);
	}

	@Override
	public void readCount(int num) {
		sqlSession.update("board.readCount", num);
	}

	@Override
	public BoardDTO content(int num) {
		return sqlSession.selectOne("board.content", num);
	}

	@Override
	public void reStepCount(BoardDTO dto) {
		sqlSession.update("board.reStepCount", dto);
	}

	@Override
	public void save(BoardDTO dto) {
		sqlSession.insert("board.save", dto);
	}

	@Override
	public BoardDTO updateNum(int num) {
		return sqlSession.selectOne("board.content", num);
	}

	@Override
	public void update(BoardDTO dto) {
		sqlSession.update("board.update", dto);
	}

	@Override
	public void delete(int num) {
		sqlSession.delete("board.delete", num);
	}

	@Override
	public String getFile(int num) { //올렸던 첨부파일을 다운로드 할때 첨부파일명을 넘겨준다.
		return sqlSession.selectOne("board.uploadFile", num);
	}

	@Override
	public List<String> getfileList(int num) {
		System.out.println("num:"+num);
		return sqlSession.selectList("board.getfileList", num);
	}

	@Override
	public void deleteref(int num) {
		sqlSession.delete("board.deleteref", num);
	}

	@Override
	public List<BoardDTO> total(BoardPageDTO pv) {
		return sqlSession.selectList("board.total", pv);
	}

}//end class
