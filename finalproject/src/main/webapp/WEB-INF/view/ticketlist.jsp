...
<body style="background-image: url('img/bg.jpg'); font-family: 'Lora'; background-attachment: fixed; background-position: center; background-size: cover;" id="body">
	...
	<section class="page-section cta" style= "background-color: rgba( 0, 0, 0, 0.5 );">
		...
		<div class="card-header" align="left">
			<span style="font-size: 20px; font-style: italic; font-weight: bold;"> &nbsp;&nbsp;<i class="fa fa-paper-plane" aria-hidden="true"></i>&nbsp;&nbsp;가는날</span>
	    </div>
		...
		<!-- 리스트 출력 -->
			...
			<!-- class="form-control"로 주고 글자를 맨 오른쪽으로 옮기어서 강제로 위치변경한다. -->
							  ...
							  <c:forEach items="${aList}" var="dto">
							   <c:if test="${dto.seat_capacity >= map.total}">
								<tr>
									...
									<!-- 왕복일 경우, 편도일 경우 -->
									<c:choose>
										<c:when test="${map.flighttype == '1'}">
											<td class="text-center">
												<c:url var="content" value="ticketlistround.do">
													<!-- 사용자 정보 파라미터 -->
												    <c:param name="route_depart_region" value="${dto.route_depart_region}"></c:param> <!-- 출발지 -->
													<c:param name="route_arrive_region" value="${dto.route_arrive_region}"></c:param> <!-- 도착지 -->
													<c:param name="route_depart_day" value="${dto.route_depart_day}"></c:param> <!-- 출발일 --> 
													...
													<c:param name="route_type" value="${dto.route_type}"></c:param> <!-- 경유구분 -->
													<c:param name="stopover_code" value="${dto.stopover_code}"></c:param> <!-- 경유지 코드 -->
													<c:param name="plane_code" value="${dto.plane_code}"></c:param> <!-- 항공기 코드 -->
													
													<!-- 티켓 찾을 때 필요한 정보 파라미터 -->
													<!-- //왕복일 때(depart_region과 arrive_region을 swap시키고 / startDate와 endDate를 swap시킨다.)  -->
													<c:param name="flighttype" value='${map.flighttype}'></c:param>
													<c:param name="depart_region" value='${map.arrive_region}'></c:param> 
													<c:param name="arrive_region" value='${map.depart_region}'></c:param> 
													<c:param name="startDate" value='${map.endDate}'></c:param> 
													<c:param name="endDate" value='${map.startDate}'></c:param> 
										...
										<c:otherwise>
											<td class="text-center">
												<c:url var="content" value="resForm.do">
												    <c:param name="route_depart_region" value="${dto.route_depart_region}"></c:param> <!-- 출발지 -->
													<c:param name="route_arrive_region" value="${dto.route_arrive_region}"></c:param> <!-- 도착지 -->
													<c:param name="route_depart_day" value="${dto.route_depart_day}"></c:param> <!-- 출발일 --> 
													<c:param name="depart_time" value="${dto.depart_time}"></c:param> <!-- 출발시간 --> 
												...
												<a href="${content}" style="text-decoration: none; font-size: 15px; font-family: sans-serif; color: #FFB400; font-weight: bold;">
													선택하기<br/><span style="color: black;">( </span><span style="color: red;">좌석 수 : ${dto.seat_capacity}</span><span style="color: black;"> )</span>
												</a>
											</td>
										</c:otherwise>
									</c:choose>
				...
				<c:if test = "${pv.startPage > 1}">
					<li class="page-item">
						<a class="page-link" aria-label="Previous" href ="ticketlist.do?currentPage=${pv.startPage - pv.blockPage}&searchKey=${pv.searchKey}&searchWord=${pv.searchWord}">
				...
				<li class= "page-item" >
					<span>
						<a class= "page-link" href="ticketlist.do?currentPage=${i}&searchKey=${pv.searchKey}&searchWord=${pv.searchWord}&flighttype=${pv.flighttype}&startDate=${pv.startDate}&endDate=${pv.endDate}&adult=${pv.adult}&children=${pv.children}&laporseat=${pv.laporseat}&depart_region=${pv.depart_region}&arrive_region=${pv.arrive_region}&airline_name=${pv.airline_name}&seat_grade=${seat_grade}">${i}</a>
					</span> 
				</li>
				...
				<c:if test = "${pv.endPage < pv.totalPage}">
					<li class="page-item">
						<a class="page-link" aria-label="Next" href ="ticketlist.do?currentPage=${pv.startPage + pv.blockPage}&searchKey=${pv.searchKey}&searchWord=${pv.searchWord}">
