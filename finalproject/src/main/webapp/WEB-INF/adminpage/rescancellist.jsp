		...
		<div class="container">
			<div class="content">
				<section id="PNWRA00001" style="display: block;">
					<div style="font-size: 45px; font-weight: bold; font-family: sans-serif;">관리자 예매확인</div>
					<br/>
					<br/>
					
					<p class="top_txt m_hide">
						<span>홈페이지에서 예매한 예매상세내역 조회 및 변경, 취소가 가능합니다.<!-- 홈페이지에서 예매한 예매상세내역 조회 및 변경, 취소가 가능합니다. --></span>
					</p>

					<div class="tab_and_sort">
						<ul class="topClBtn m_hide">
							<li><a href="javascript:reserve();"
								class="btn btn01 btn-color-w1" id="list_btn">예매내역<!-- 예매내역 --></a></li>
							<li><a href="javascript:cancel();"
								class="btn btn01 btn-color-w1 on" id="cancel_btn">취소내역<!-- 취소내역 --></a></li>
						</ul>
					...
					<table class="bbs_list reslist" id="reslist">
						...
						<thead>
							<tr>
								...
								<th scope="col">여정<!--여정--></th>
								<th scope="col">상태<!--상태--></th>
						...
						<tbody id="PNWRA00001_bookingList">
							...
							<c:forEach items="${aList}" var="dto">
								<tr>
									<td><a href="javascript:PNWRA00001_doMyBookingDetail(&#39;YEML8E&#39;);" class="red">${dto.reserve_code}</a></td>
										
									<c:if test="${dto.ticket_type eq 1 }">
										<td>왕복</td>
									</c:if>
									
									<c:if test="${dto.ticket_type eq 2 }">
										<td>편도</td>
									</c:if>
									
									<td>${dto.reserve_depart_day}</td>
									
									<td>${dto.reserve_journey}</td>
									
									<td> <span style="color: red;">${dto.reserve_state}</span></td>
									
									<td>
										<c:url var="content" value="adminrescancelDetail.do">
											<c:param name="currentPage" value="${pv.currentPage}" />
											<c:param name="reserve_code" value="${dto.reserve_code}" />
										</c:url> 
										
										<a href="${content}" class="btn btn03 btn-color04">상세보기</a>
									</td>
						...
						<tfoot>
							<tr>
								<td colspan="6" style="text-align: center">총 매출액(예약완료) &nbsp;: &nbsp;<fmt:formatNumber
					...
					<ul class="pagination justify-content-center">
						...
						<c:if test="${pv.startPage > 1}">
							<li class="page-item"><a class="page-link"
								aria-label="Previous"
								href="adminrescancelList.do?currentPage=${pv.startPage - pv.blockPage}">
						...
						<c:forEach begin="${pv.startPage}" end="${pv.endPage}" var="i">
							...
							<li class="page-item"><span> <a class="page-link"
									href="adminrescancelList.do?currentPage=${i}">${i}</a>
							</span></li>
