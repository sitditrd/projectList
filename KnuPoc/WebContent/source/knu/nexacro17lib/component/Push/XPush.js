	...
	_pXPush._on_xpush = function (recv) {
		...
		for (var j = 0; j < this.commandlist.length; j++) {
			...
			if (cc.messagetype == recv.type) {
				...
				var dlidx;
				var data, layoutKey, messageKey;

				for (dlidx = 0; dlidx < datalistlen; dlidx++) {
					data = recv.datalist[dlidx];
					layoutKey = data.id;
				...
				for (dlidx = 0; dlidx < datalistlen; dlidx++) {
					...
					if (cc.type == "append") {
						...
						if (cc.row < cc.objdataset.getRowCount()) {
							value = cc.objdataset.getColumn(cc.row, colIdx);
							if (value != data.item) {
						...
						paramAllColumns.push(data.id);
						value = cc.objdataset.getColumn(cr, colIdx);
						if (value != data.item) {
							callfunc1 = true;
							callfunc = true;
						...
						if (paramChangeRowsTemp.length == 0) {
							for (cr = 0; cr < cc.objdataset.getRowCount(); cr++) {
